...
node_re = re.compile(r"^\d+:\s+(.+) \[node \d+\] - \d+")
node_ip_re = r"\*.+ has joined the game \(player {}\) \(([\d\.]+)[:\)]"
action_re = re.compile(
    r"^((\*.+ entered the game\.)|(\*.+ left the game)|(\*.+ has joined the game \(player \d+\))|(\*.+ renamed to [^\n]+)|(\*.+ became a spectator\.)|(.+ has finished the race\.)|(.+ ran out of time\.)|(The round has ended\.)|(Speeding off to level\.\.\.))"
)
    ...
    async with aiohttp.ClientSession() as session:
        while True:
            ...
            try: ## TESTE PRA NAO CAIR O BRIDGE TODA HORA
                ...
                with open(f"{config['server_folder_path']}latest-log.txt", "r", encoding='utf8', errors='ignore') as f:
                    ...
                    if last_log_line != 0:
                        for line in log[last_log_line:]:
                            if line.startswith("[HITFEED] "):
                                ...
                                embed.set_image(url="http://144.22.60.203:8000/maps/" + mapid + "-RR.PNG")
                                embed.url = "http://144.22.60.203:8000/maps/" + mapid + ":" + mapname
                                ...
                                contest = list(
                                    sorted(
                                        filter(lambda x: x[3] != 0, players), key=lambda x: (x[3], x[2])
                                    )
                                )
...
async def on_message(message):
    if message.channel.id == config["chat_bridge_channel_id"]:
        if not message.author.bot:
            ...
            text = text.encode('ascii', 'ignore').decode('ascii') # remove non-ASCII characters to not crash srb2k/RR lol
    ...
    if status == "ON":
        ...
        for _ in range(5):
            with open(f"{config['server_folder_path']}latest-log.txt", "r", encoding='utf8', errors='ignore') as f:
                ...
                for line in log:
                    if state == 0:
                        if line.startswith("Ring Racers"):
                            state = 1
                    elif state == 1:
